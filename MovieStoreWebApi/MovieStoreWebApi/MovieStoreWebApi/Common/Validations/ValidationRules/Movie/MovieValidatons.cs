...
namespace MovieStoreWebApi.Common.Validations.ValidationRules
{
    public class MovieValidatons : AbstractValidator<MovieCreateDto>
    {
        ...
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("Film adı boş geçilemez.").MaximumLength(100).WithMessage("Film adı 60 karakterden az olmalıdır.");
            ...
            RuleFor(x => x.Price).NotEmpty().WithMessage("Film fiyatı boş geçilemez.").GreaterThan(0).WithMessage("Film fiyatı sıfırdan büyük olmalıdır.");
